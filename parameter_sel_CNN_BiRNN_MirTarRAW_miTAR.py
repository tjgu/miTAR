...
inputf = "data/data_miRaw_DeepMirTar_equal_training_noRepeats_3folds_2.txt"
...
for batch in batches:
	for lr in learning_rate:
		for dout in dropout:
			...
			mcp = ModelCheckpoint(filepath='results/miTAR_CNN_BiRNN_b' + str(batch) + '_lr' + str(lr) + '_dout' + str(dout) + '.h5', monitor='val_acc', mode='max', save_best_only=True, verbose=1)
			...
			bestModel = load_model('results/miTAR_CNN_BiRNN_b' + str(batch) + '_lr' + str(lr) + '_dout' + str(dout) + '.h5')
