...
from utils import formatmiRAW, padding, convert3D, flatten
...
for batch in batches:
	for lr in learning_rate:
		for dout in dropout:
			...
			mcp = ModelCheckpoint(filepath='results/miTAR2_CNN_BiRNN_b' + str(batch) + '_lr' + str(lr) + '_dout' + str(dout) + '.h5', monitor='val_acc', mode='max', save_best_only=True, verbose=1)
			...
			bestModel = load_model('results/miTAR2_CNN_BiRNN_b' + str(batch) + '_lr' + str(lr) + '_dout' + str(dout) + '.h5')
