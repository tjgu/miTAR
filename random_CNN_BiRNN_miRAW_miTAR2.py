...
from utils import formatmiRAW, padding, convert3D, flatten, evals

from sklearn.model_selection import train_test_split

# prepare the input data
inputf = "data/data_miRaw_noL_noMisMissing.txt"
seqs, label =  formatmiRAW(inputf)
...
for seed in seeds:
	...
	mcp = ModelCheckpoint(filepath='results/miTAR2_CNN_BiRNN_b' + str(batch) + '_lr' + str(lr) + '_dout' + str(dout) + '_seed' + str(seed) + '_1122.h5', monitor='val_acc', mode='max', save_best_only=True, verbose=1)
	...
	bestModel = load_model('results/miTAR2_CNN_BiRNN_b' + str(batch) + '_lr' + str(lr) + '_dout' + str(dout) + '_seed' + str(seed) + '_1122.h5')
...
outfName = 'results/miTAR2_CNN_BiRNN_b' + str(batch) + '_lr' + str(lr) + '_dout' + str(dout) + '_seeds_1122_evals.txt'
